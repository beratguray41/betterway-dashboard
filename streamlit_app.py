...
# =========================================================
# 0) FİRMA KONFİG (ŞİFRE -> SHEET + GID)
# =========================================================
FIRMS = {
    "Karınca2025.": {
...
# =========================================================
# 7) VERİ ÇEKME (firma bazlı sheet_id + gid)
# =========================================================
SHEET_ID = st.session_state.get("sheet_id")
HAS_ISE_ALIM = bool(st.session_state.get("has_ise_alim", False))
GIDS = st.session_state.get("gids", {})

GENEL_GID = GIDS.get("genel", "0")
SURUCU_GID = GIDS.get("surucu", "395204791")
HATA_OZETI_GID = GIDS.get("hata", "2078081831")
...
with st.sidebar:
    ...
    st.markdown(f"""
        <div style="background:rgba(255,255,255,0.05); padding:15px; border-radius:12px; margin-bottom:20px; border:1px solid rgba(255,255,255,0.1);">
            ...
            <div style="color:white; font-weight:700; font-size:14px; margin-top:4px;">{esc(st.session_state.get('firm', 'Müşteri'))}</div>
        </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown("---")
    st.caption("BetterWay Intelligence v6.5")

# =========================================================
# 9) SÜRÜCÜ SORGULAMA (✅ TEK HTML BLOK – div asla dağılmaz)
# =========================================================
if (
    menu == "🔍 Sürücü Sorgula"
    and secilen_surucu != "Seçiniz..."
    and not df_surucu.empty
    and "Sürücü Adı" in df_surucu.columns
):
    ...
    <div class="hero-profile">
      <div style="display:flex; justify-content:space-between; align-items:center;">
        <div>
          ...
          <p style="color:#94a3b8; font-size:18px;">
            <span style="margin-right:18px;">📍 {egitim_yeri}</span>
            <span style="margin-right:18px;">🎓 {egitim_turu}</span>
            <span>📅 {egitim_tarih}</span>
          </p>
        </div>
      ...
      <div style="display:grid; grid-template-columns:1fr 1fr; gap:40px; margin-top:40px;">
        ...
        <div class="glass-card">
          ...
          <p style="color:#cbd5e1; line-height:1.6;">{zayif_html}</p>
        </div>
      </div>
...
# =========================================================
# 10) GENEL BAKIŞ (KPI + pasta + yenileme + arşiv)
# =========================================================
else:
    if HAS_ISE_ALIM:
    ...
    with col_l:
        ...
        if not df_hata.empty and df_hata.shape[1] >= 2:
            ...
            fig.update_traces(textposition="inside", textinfo="percent", marker=dict(line=dict(color="#0f1115", width=3)))
        ...
        if not df_surucu.empty and "EĞİTİM YENİLEMEYE KAÇ GÜN KALDI?" in df_surucu.columns:
            ...
            if not crit_df.empty:
                for _, rrr in crit_df.head(4).iterrows():
                    st.markdown(f"""<div class="status-alert">🚨 {esc(rrr.get('Sürücü Adı','-'))} - <span style="float:right;">{int(rrr.get('kg',0))} Gün</span></div>""", unsafe_allow_html=True)
            else:
